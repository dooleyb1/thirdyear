...
/*!50013 DEFINER=`admin_rw` SQL SECURITY DEFINER */
/*!50001 VIEW `managers_overview` AS select `m`.`manager_name` AS `manager_name`,`t`.`team_name` AS `team_name`,`m`.`salary` AS `salary` from (`team` `t` join `manager` `m`) where (`t`.`team_id` = `m`.`team_id`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`admin_rw` SQL SECURITY DEFINER */
/*!50001 VIEW `teams_overview` AS select `t`.`team_name` AS `team_name`,`m`.`manager_name` AS `manager_name`,`s`.`stadium_name` AS `stadium_name` from ((`team` `t` join `manager` `m`) join `stadium` `s`) where ((`t`.`team_id` = `m`.`team_id`) and (`t`.`team_id` = `s`.`team_id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-11-19 11:09:12
